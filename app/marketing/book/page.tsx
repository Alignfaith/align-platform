...
export default function BookPage() {
  return (
    <>
      {/* Hero */}
      <section style={{ backgroundColor: '#ffffff', padding: '80px 24px', borderBottom: '1px solid #E5E7EB' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <div style={{ position: 'relative' }}>
              <div style={{
                ...
                background: 'radial-gradient(circle, rgba(192,24,42,0.12) 0%, transparent 70%)',
              ...
              <Image
                ...
                style={{ borderRadius: '8px', boxShadow: '0 20px 50px rgba(0,0,0,0.2)', position: 'relative', zIndex: 1 }}
                priority
              />
            </div>
          </div>
          ...
          <div>
            ...
            <p style={{ color: '#9CA3AF', fontSize: '1rem', marginBottom: '20px', fontStyle: 'italic' }}>
              Preparing yourself for the love you desire — by Thomas Marks
            </p>
            <p style={{ color: '#374151', lineHeight: 1.8, fontSize: '0.975rem', marginBottom: '32px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              <a href={BOOK_URL} target="_blank" rel="noopener noreferrer" style={{
                backgroundColor: '#c0182a', color: '#fff', padding: '13px 28px',
              ...
              <a href={`${APP_URL}/register`} style={{
                ...
                color: '#374151', padding: '13px 28px', borderRadius: '8px',
      ...
      {/* What you'll learn */}
      <section style={{ backgroundColor: '#F9FAFB', padding: '80px 24px', borderBottom: '1px solid #E5E7EB' }}>
        <div style={{ maxWidth: '840px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', fontFamily: 'var(--font-heading)', fontWeight: 700, color: '#111827', marginBottom: '12px' }}>
              What you&apos;ll walk away with
            </h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', maxWidth: '640px', margin: '0 auto' }}>
            {outcomes.map((o, i) => (
              <div key={i} style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', backgroundColor: '#ffffff', border: '1px solid #E5E7EB', borderRadius: '10px', padding: '16px 20px' }}>
                <div style={{
                  width: '22px', height: '22px', borderRadius: '50%',
                  backgroundColor: '#FEF2F2', border: '1px solid #FECACA',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  flexShrink: 0, marginTop: '1px', fontSize: '0.7rem', color: '#c0182a', fontWeight: 700,
                }}>✓</div>
                <p style={{ color: '#374151', fontSize: '0.975rem', lineHeight: 1.65 }}>{o}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {chapters.map((c) => (
              <div key={c.num} style={{
                ...
                padding: '18px 20px', borderRadius: '10px', border: '1px solid #E5E7EB',
                backgroundColor: '#ffffff',
              }}>
                <span style={{ color: '#D1D5DB', fontSize: '0.8rem', fontWeight: 700, paddingTop: '2px' }}>
                  {String(c.num).padStart(2, '0')}
                </span>
                <div>
                  <p style={{ color: '#111827', fontWeight: 600, fontSize: '0.95rem', marginBottom: '4px' }}>{c.title}</p>
                  <p style={{ color: '#9CA3AF', fontSize: '0.875rem', lineHeight: 1.55 }}>{c.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ backgroundColor: '#F9FAFB', padding: '80px 24px', borderTop: '1px solid #E5E7EB' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontFamily: 'var(--font-heading)', fontWeight: 700, color: '#111827' }}>Reader reviews</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '20px' }}>
            {reviews.map((r) => (
              <div key={r.name} style={{
                backgroundColor: '#ffffff', border: '1px solid #E5E7EB',
                borderRadius: '14px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)',
              }}>
                ...
                <p style={{ color: '#6B7280', fontWeight: 600, fontSize: '0.875rem' }}>— {r.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '520px', margin: '0 auto' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.85)', lineHeight: 1.7, marginBottom: '32px' }}>
