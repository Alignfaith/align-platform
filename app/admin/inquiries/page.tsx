...
export default function InquiriesPage() {
  ...
  const columns: Column<Inquiry>[] = [
    {
      ...
      render: i => (
        <div>
          ...
          <div style={{ color: 'var(--color-text-tertiary)', fontSize: 'var(--text-xs)' }}>{i.email}</div>
        </div>
      ),
    },
    {
  ...
  return (
    <>
      <AdminPageHeader title="Inquiries" breadcrumbs={[{ label: 'Inquiries' }]} />
      ...
      {loading ? (
        ...
        <>
          <DataTable<Inquiry> columns={columns} data={inquiries} keyField="id" emptyMessage="No inquiries" onRowClick={openInquiry} />
          ...
          <div style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-6)', maxWidth: '560px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-5)' }}>
              <div>
                ...
                <div style={{ marginTop: 'var(--space-1)', display: 'flex', gap: 'var(--space-3)', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
              <a
                href={`mailto:${selected.email}?subject=Re: Matching Service Inquiry`}
                onClick={() => updateStatus(selected.id, 'REPLIED')}
                style={{ flex: 1, minWidth: '120px', padding: 'var(--space-2) var(--space-4)', background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.3)', color: '#16a34a', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-sm)', fontWeight: 'var(--font-semibold)', textDecoration: 'none', textAlign: 'center', cursor: 'pointer' }}
              >
                Reply via Email
              </a>
              {selected.status !== 'ARCHIVED' && (
                <button
                  onClick={() => { updateStatus(selected.id, 'ARCHIVED'); setSelected(null) }}
                  disabled={updating}
